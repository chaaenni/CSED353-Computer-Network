...
void Router::route_one_datagram(InternetDatagram &dgram) {
    ...
    for(auto i = _routing_table.begin(); i != _routing_table.end(); i++){
        ...
        if(prefix_length == 0) mask = 0;
            ...
            //finally, mask = 11111111 11111111 10000000 00000000 (it can keep only the most significant prefix_length bits of dgram_dst_addr)
    ...
    if(longest_prefix_len == -1) return;
    ...
    if(next_hop.has_value()){//if the next_hop is not empty, then it conatins the IP address of the next router along the path
